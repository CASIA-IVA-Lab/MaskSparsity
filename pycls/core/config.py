...
#--------------------------------------------------------------------------------------#
_C.SPARSE = CfgNode()
_C.SPARSE.NORM_LAYER_NAMES = []
_C.SPARSE.SPARSITY = None
_C.SPARSE.MASK_DICT = None
_C.SPARSE.REGULAR_METHOD = 'L1'

#-------------------------------------------------------------------------------------#
#prune keys
#-------------------------------------------------------------------------------------#
_C.PRUNE = CfgNode()
_C.PRUNE.FACTOR  = None
_C.PRUNE.GET_MASK = False
_C.PRUNE.USE_MASK = None

_C.PRUNE.THRESHOLD_METHOD = 'th'
_C.PRUNE.MIN_CHANNELS = 1
_C.PRUNE.SPUER_NORM_LAYERS = []
_C.PRUNE.SPUER_SUCC_LAYERS = []
_C.PRUNE.SPUER_PREC_LAYERS = []
_C.PRUNE.PRUNE_RESIDUAL = False
...
def load_cfg_fom_args(description="Config file options.", args=None):
    ...
    if _C.TRAIN.WEIGHTS != "" :
       aa=_C.TRAIN.WEIGHTS.strip( ' \n' ).split('/' )
       aa1= '/'.join(aa[:-1])
       aa2= '/'.join(aa[:-2])
       if _C.SPARSE.SPARSITY is not None:#sparse
           if _C.SPARSE.MASK_DICT is None :
               _C.OUT_DIR = os.path.join(aa2,"sparsity_train",_C.SPARSE.REGULAR_METHOD,"lamuda_"+str(_C.SPARSE.SPARSITY))
           else :
               bb= _C.SPARSE.MASK_DICT.strip( ' \n' ).split('/' )[-1]
               _C.OUT_DIR = os.path.join(aa2,"sparsity_train",_C.SPARSE.REGULAR_METHOD, bb ,"lamuda_"+str(_C.SPARSE.SPARSITY))
       elif  _C.PRUNE.FACTOR is not None:#prune
           bb= 'prune_'
           if _C.PRUNE.MIN_CHANNELS == 0 :
               bb += "block_"
           if _C.PRUNE.PRUNE_RESIDUAL :
               bb += "residual_" 
           bb= bb+ _C.PRUNE.THRESHOLD_METHOD +"-p"+  str(_C.PRUNE.FACTOR)
           _C.OUT_DIR = os.path.join(aa2,"prune_model",bb)
       elif _C.PRUNE.USE_MASK is not None:
           bb= _C.PRUNE.USE_MASK.strip( ' \n' ).split('/' )[-1]
           _C.OUT_DIR = os.path.join(aa2,"prune_model",'usemask'+bb)

       elif  _C.FINETUNE  :#finetune
           _C.OUT_DIR = os.path.join(aa1 , "finetune" , "lr_" + str(_C.OPTIM.BASE_LR))
       elif _C.SCRATCH :#scratch
           _C.OUT_DIR = os.path.join(aa1 , "scratch" , "lr_" + str(_C.OPTIM.BASE_LR))
       elif  0:#scratch
           pass
       else:#normal_train
           pass            
